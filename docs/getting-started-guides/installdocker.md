...
上一章节[创建Inventory文件](docs/getting-started-guides/inventory.md)我们为playbook 中的每一个 play创建好了目标机或者组,下面我们将以这些用户身份去完成要执行的步骤（called tasks）.在 ansible 中,play 的内容,被称为 tasks,即任务.这里我们为每一个play创建了一个task列表，一个 task 在其所对应的所有主机上执行完毕之后,下一个 task 才会执行.有一点需要明白的是（很重要）,在一个 play 之中,所有 hosts 会获取相同的任务指令,这是 play 的一个目的所在,也就是将一组选出的 hosts 映射到 task, example:
```ssh
...
* 创建playbook中的“play”-base.yml,然后指定主机组执行动作
---
```ssh
- hosts: machines
  roles:
    - base
```
> 这里指定machines主机组执行动作,hosts 行的内容是一个或多个组或主机的 patterns,以逗号为分隔符
